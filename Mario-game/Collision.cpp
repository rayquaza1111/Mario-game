	...
	int filterY = 1)			// 1 = process events on Y-axis, 0 = skip events on Y
{
	...
	for (UINT i = 0; i < coEvents.size(); i++)
	{
		...
		if (filterBlock == 1)
		{
			...
			if (!c->obj->IsBlocking())
				continue;
			//block from top direction
			/*int t;
			c->obj->GetFilterBlockTop(t);
			if (c->ny < 0 && !t)
				continue;*/
			//block from left direction
			int l;
			c->obj->GetFilterBlockLeft(l);
			if (c->nx < 0 && !l)
				continue;
			//block from right direction
			int r;
			c->obj->GetFilterBlockRight(r);
			if (c->nx > 0 && !r)
				continue;
			//block from bottom direction
			int b;
			c->obj->GetFilterBlockBottom(b);
			if (c->ny > 0 && !b)
				continue;
		}
